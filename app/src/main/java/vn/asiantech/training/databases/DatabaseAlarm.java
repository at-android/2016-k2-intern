package vn.asiantech.training.databases;
...
public class DatabaseAlarm {
    ...
    public ArrayList<Alarm> getData() {
        ...
        for (c.moveToFirst(); !c.isAfterLast(); c.moveToNext()) {
            alarm = new Alarm();
            alarm.setmId(c.getInt(iRow));
            alarm.setmHour(c.getString(iHour));
            alarm.setmMin(c.getString(iMin));
            alarm.setmRepeart(c.getString(iDays));
            alarm.setmRepeartChar(c.getString(iDayChar));
            if (c.getString(iStatus).equals("true"))
                alarm.setmStatus(true);
            else
                alarm.setmStatus(false);
            arrInfo.add(alarm);
